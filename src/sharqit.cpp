 ...
 */
void optimize(std::string& fin, Sharqit::OptimizerKind kind)
{
  try {
    Sharqit::Optimizer opt;
    Sharqit::QCirc qc_in;
    qc_in.load(fin);
    Sharqit::QCirc qc_out = opt.execute(qc_in, kind);
    qc_out.print_qcirc();
...
{
  Sharqit::OptimizerKind kind = Sharqit::OptimizerKind::ZXCalculus;
  ...
  for (int n=1; n<argc; n++) {
    if (strcmp(argv[n],"--help") == 0) {
      ...
      if (token.size() == 1) kind = Sharqit::OptimizerKind::ZXCalculus;
      else if (token[1] == "zx") kind = Sharqit::OptimizerKind::ZXCalculus;
      else if (token[1] == "pp") kind = Sharqit::OptimizerKind::PhasePolynomial;
      else {
