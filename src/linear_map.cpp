...
{
  Sharq::LinearMap lmap = *this;

  for (uint32_t i = 0; i < row_num(); ++i) {
    for (uint32_t j = 0; j < col_num(); ++j) {
      if (std::abs(lmap(i, j).real()) > Sharq::EPS) return false;
      else if (std::abs(lmap(i, j).imag()) > Sharq::EPS) return false;
    }
  }
  return true;
}

bool Sharq::LinearMap::is_identity() const
{
  if (row_num() != col_num()) return false;
  else if (row_num() == 0 || col_num() == 0) return false;

  Sharq::LinearMap lmap = *this;
  Sharq::LinearMap lmap_identity(row_num(), col_num());

  lmap -= lmap_identity;
  return lmap.is_zero();
}

bool Sharq::LinearMap::is_diagonal() const
{
  if (row_num() != col_num()) return false;
  else if (row_num() == 0 || col_num() == 0) return false;
...
bool Sharq::LinearMap::is_identity_multiple_constant() const
{
  if (row_num() != col_num()) return false;
  else if (row_num() == 0 || col_num() == 0) return false;
  ...
  for (uint32_t i = 0; i < row_num(); ++i) {
    if (std::abs(lmap(i, i) - lmap(0, 0)) > Sharq::EPS) return false;
  }
  return true;
}

bool Sharq::LinearMap::is_unitary() const
{
  if (row_num() != col_num()) return false;
  else if (row_num() == 0 || col_num() == 0) return false;
...
bool Sharq::LinearMap::is_equal(const LinearMap& other) const
{
...
Sharq::LinearMap& Sharq::LinearMap::operate_qgate(const QGate& qgate)
{
  if (row_num() != col_num()) {
