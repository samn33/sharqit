...
TEST(LinearMap, Constructor03) {
  Sharqit::LinearMap lmap({{1.0, 2.0}, {3.0, 4.0}});
...
TEST(LinearMap, Add01) {
  Sharqit::LinearMap lmap_A({{10.0, 20.0},
			   {30.0, 40.0}});
  Sharqit::LinearMap lmap_B({{1.0, 2.0},
			   {3.0, 4.0}});
  Sharqit::LinearMap lmap_C = lmap_A + lmap_B;
...
TEST(LinearMap, Add02) {
  ...
  Sharqit::LinearMap lmap_A({{10.0 + 2.0 * I, 20.0 + 2.0 * I},
			   {30.0 + 2.0 * I, 40.0 + 2.0 * I}});
  Sharqit::LinearMap lmap_B({{1.0 + I, 2.0 + I},
			   {3.0 + I, 4.0 + I}});
  Sharqit::LinearMap lmap_C = lmap_A + lmap_B;
...
TEST(LinearMap, Mul03) {
  ...
  Sharqit::LinearMap lmap_C = lmap_A * B;
...
TEST(LinearMap, Mul04) {
  ...
  Sharqit::LinearMap lmap_A({{10.0 + 2.0 * I, 20.0 + 2.0 * I},
			   {30.0 + 2.0 * I, 40.0 + 2.0 * I}});
  std::complex<double> B(1.0 - 2.0 * I);
  Sharqit::LinearMap lmap_C = B * lmap_A;
...
TEST(LinearMap, IsZero01) {
  std::complex<double> I(0.0, 1.0);
  Sharqit::LinearMap lmap({{-1.0 - 2.0 * I, 1.0e-9 + 1.0e-9 * I},
			 {-3.0 - 2.0 * I, -4.0 - 2.0 * I}});
...
TEST(LinearMap, IsZero02) {
  std::complex<double> I(0.0, 1.0);
  Sharqit::LinearMap lmap({{1.0e-9 + 1.0e-9 * I, 0.0},
			 {0.0, -1.0e-9 - 1.0e-9 * I}});
...
TEST(LinearMap, IsIdentity01) {
  std::complex<double> I(0.0, 1.0);
  Sharqit::LinearMap lmap({{1.0 + 1.0e-9 * I, -1.0e-9 * I},
			 {0.0, 1.0}});
...
TEST(LinearMap, IsIdentity02) {
  std::complex<double> I(0.0, 1.0);
  Sharqit::LinearMap lmap({{10.0 + 1.0e-9 * I, -1.0e-9 * I},
			 {0.0, 10.0}});
...
TEST(LinearMap, IsIdentity03) {
  Sharqit::LinearMap lmap(2, 3);
  EXPECT_EQ(lmap.is_identity(), false);
}

TEST(LinearMap, IsEqual01) {
  std::complex<double> I(0.0, 1.0);
  Sharqit::LinearMap lmap_A({{1.0 + 2.0 * I, 2.0 + 3.0 * I},
			   {4.0 + 5.0 * I, 6.0 + 7.0 * I}});
  Sharqit::LinearMap lmap_B({{1.0 + 2.0 * I, 2.0 + 3.0 * I},
			   {4.0 + 5.0 * I, 6.0 + 7.0 * I}});
...
TEST(LinearMap, IsDiagonal01) {
  std::complex<double> I(0.0, 1.0);
  Sharqit::LinearMap lmap({{1.0 + 2.0 * I, 0.0},
			   {0.0, 6.0 + 7.0 * I}});
...
TEST(LinearMap, IsDiagonal02) {
  std::complex<double> I(0.0, 1.0);
  Sharqit::LinearMap lmap({{1.0 + 2.0 * I, -5.0},
			   {0.0, 6.0 + 7.0 * I}});
...
TEST(LinearMap, IsIdentityMultipleConstant02) {
  std::complex<double> I(0.0, 1.0);
  Sharqit::LinearMap lmap({{2.0 + 2.0 * I, 0.0},
			 {0.0, 1.0 + 2.0 * I}});
...
TEST(LinearMap, IsIdentityMultipleConstant03) {
  std::complex<double> I(0.0, 1.0);
  Sharqit::LinearMap lmap({{1.0 + 2.0 * I, 3.0},
			 {0.0, 1.0 + 2.0 * I}});
  EXPECT_EQ(lmap.is_identity_multiple_constant(), false);
}
...
TEST(LinearMap, IsUnitary01) {
  ...
  qc.h(0);
  qc.cx(0,1);
  Sharqit::LinearMap lmap = qc.to_linearmap();
